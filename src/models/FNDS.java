...
public class FNDS {
    ...
    public List<List<Individuo>> execute(ArrayList<Individuo> individuos){
        ...
        for (int i = 0; i < pontos.size(); i++) {
            ...
            ponto_p.S = new ArrayList<Ponto>();
            ponto_p.n = 0;
            for (int j = 0; j < pontos.size(); j++) {
                if (i != j){
                   Ponto ponto_q = pontos.get(j);
                   if (domina(ponto_p.genes,ponto_q.genes)){
                        ponto_p.S.add(ponto_q);
                   }else if (domina(ponto_q.genes,ponto_p.genes)){
                       ponto_p.n++;
                   }
                }
            }
    ...
    public boolean domina(double[] avalInd,double[] avalInd2){
